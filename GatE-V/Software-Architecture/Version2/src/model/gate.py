...
class MultilevelFGTQGate(nn.Module):
    ...
    # R50 backbone output channels for stages [1, 2, 3] (return_idx)
    DEFAULT_CHANNELS = {"P3": 512, "P4": 1024, "P5": 2048}
    ...
    def forward(
        ...
        Args:
            ...
            levels: Which levels to gate. Defaults to all registered levels.

        Returns:
            Gated feature maps in the same order.
        """
        ...
        return [
            self.forward_level(feat, task_ids, lvl)
...
def get_task_semantic_seeding(dim: int = 128) -> torch.Tensor | None:
    """Generate CLIP-seeded task embeddings via JL projection.
    ...
    print("[model] Generating semantic seeding via CLIP text encoder...")
