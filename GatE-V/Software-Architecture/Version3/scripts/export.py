...
Usage:
    python scripts/export.py --config configs/gatev_base.yaml \\
                              --checkpoint runs/gatev_base/gatev_edge_detect_final.pth

Output: <checkpoint_dir>/gatev_exported.onnx
"""
...
def main():
    ...
    output = Path(args.output) if args.output else ckpt_path.parent / "gatev_exported.onnx"
    ...
    with torch.no_grad():
        pred_logits, pred_boxes, task_logits, exist_logits, *_ = model(dummy_image, dummy_task)
