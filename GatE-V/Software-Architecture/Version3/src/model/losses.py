...
def _compute_core_losses(pred_logits, pred_boxes, targets, device,
    ...
    Supports configurable Hungarian costs and Matchability-Aware Loss (MAL).
    When mal_alpha > 0, classification loss is weighted by (matched_IoU)^mal_alpha.
    """
    ...
    # ── Per-box focal loss (needed for MAL weighting) ──
    ...
    per_box_focal = alpha_t * (1.0 - p_t) ** 2.0 * bce  # [B, Q, 1]
    ...
    if pred_matched:
        ...
        # MAL: weight positive classification by IoU quality ^ mal_alpha
        if mal_alpha > 0.0:
            matched_iou = torch.diag(giou_matrix).clamp(min=1e-6)
            qw = (matched_iou ** mal_alpha).detach()
            qw_matrix = torch.ones(B, Q, 1, device=device)
            offset = 0
            for b, (pred_idx, _) in enumerate(indices):
                n = len(pred_idx)
                if n > 0:
                    qw_matrix[b, pred_idx] = qw[offset:offset + n].view(-1, 1)
                    offset += n
