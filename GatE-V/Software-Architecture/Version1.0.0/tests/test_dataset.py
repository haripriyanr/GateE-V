...
def test_yaml_loading():
    """Load from YAML file."""
    yaml_path = Path(__file__).resolve().parent.parent / "configs" / "gatev_base.yaml"
    if not yaml_path.exists():
        print("⊘ test_yaml_loading (skipped — config file not found)")
        return
    cfg = GatEVConfig.from_yaml(yaml_path)
    cfg.validate()
...
def test_overrides():
    ...
    if not yaml_path.exists():
        ...
        return
    cfg = GatEVConfig.from_yaml(yaml_path, overrides=["training.batch_size=4"])
