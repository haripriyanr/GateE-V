...
Handles environment setup and delegates to the modular pipeline:
  ...
  o → optimize    threshold search
  v → visualize   CPU comparison GUI
  p → precompute  resize images
...
Direct CLI (non-interactive):
  ...
  python scripts/eval.py  --checkpoint runs/gatev_base/checkpoints/best.pth
  python scripts/export.py --checkpoint runs/gatev_base/gatev_edge_detect_final.pth
"""
...
def _resolve_lfs_url(owner: str, repo: str, path: str, branch: str, fetch_json) -> str | None:
    """Resolve an LFS pointer file to its actual content URL via GitHub Contents API."""
    contents_url = f"https://api.github.com/repos/{owner}/{repo}/contents/{quote(path)}?ref={branch}"
    try:
        info = fetch_json(contents_url)
        return info.get("download_url")
    except Exception:
        return None


def _repo_owner_name(repo_slug: str) -> tuple[str, str]:
...
def mirror_dataset(target_dir: Path, force: bool = False) -> None:
    ...
    for item in tree.get("tree", []):
        ...
        # Try raw download first (fast for non-LFS files)
        raw_url = f"https://raw.githubusercontent.com/{DATASET_REPO}/{DATASET_BRANCH}/{quote(path)}"
        try:
            req = Request(raw_url, headers={"User-Agent": "GatE-V"})
        ...
        # If it's an LFS pointer, resolve via Contents API
        if _is_lfs_pointer(raw_data):
            owner, repo = _repo_owner_name(DATASET_REPO)
            dl_url = _resolve_lfs_url(owner, repo, path, DATASET_BRANCH, fetch_json)
            if not dl_url:
                print(f"[dataset]  Skipping {path} (LFS resolution failed)")
                continue
            print(f"[dataset]  Resolved LFS: {path}")
            try:
                with urlopen(Request(dl_url, headers={"User-Agent": "GatE-V"})) as r, \
                     dest.open("wb") as f:
                    import shutil as sh; sh.copyfileobj(r, f)
            except Exception as e:
                print(f"[dataset]  Error downloading LFS content {path}: {e}")
                continue
        else:
...
def download_coco_images(dataset_dir: Path, images_dir: Path) -> None:
    ...
    COCO_DIRECT = "http://images.cocodataset.org"

    def _dl(img_info, dest):
        tgt = dest / img_info["file_name"]
        if tgt.exists() and tgt.stat().st_size > 0: return
        fname = img_info["file_name"]
        # Construct direct URL from file_name (coco_url is dead mscoco.org)
        if fname.startswith("COCO_train2014_"):
            url = f"{COCO_DIRECT}/train2014/{fname}"
        elif fname.startswith("COCO_val2014_"):
            url = f"{COCO_DIRECT}/val2014/{fname}"
        elif fname.startswith("COCO_train2017_"):
            url = f"{COCO_DIRECT}/train2017/{fname}"
        elif fname.startswith("COCO_val2017_"):
            url = f"{COCO_DIRECT}/val2017/{fname}"
        else:
...
def run_train() -> None:
    _run_script("train.py", "--config", "configs/gatev_base.yaml")


def run_eval() -> None:
    # Find best checkpoint automatically
    best = ROOT_DIR / "runs" / "gatev_base" / "checkpoints" / "best.pth"
    final = ROOT_DIR / "runs" / "gatev_base" / "gatev_edge_detect_final.pth"
    ...
    if ckpt is None:
        ...
        return
    _run_script("eval.py", "--config", "configs/gatev_base.yaml", "--checkpoint", ckpt)
...
def run_export() -> None:
    ...
    if ckpt is None:
        ...
        return
    _run_script("export.py", "--config", "configs/gatev_base.yaml", "--checkpoint", ckpt)


def run_precompute() -> None:
    # Uses 800x800 as that is the configured full resolution
    _run_script("precompute_images.py", "--src_dir", "./data/images", "--dst_dir", "./data/images_800", "--img_size", "800")
...
def run_optimize() -> None:
    ...
    if ckpt is None:
        ...
        return
    _run_script("optimize_gating.py", "--config", "configs/gatev_base.yaml", "--checkpoint", ckpt)
...
def main() -> int:
    ...
    if "--download-images" in sys.argv:
        ...
        return 0
    if "--precompute-images" in sys.argv:
        run_precompute()
        return 0
