...
def train_stage(
    ...
    lr_sched_type = getattr(cfg.training, "lr_scheduler", "cosine")
    if lr_sched_type == "cosine_warm_restarts":
        lr_T0 = getattr(cfg.training, "lr_T0", 15)
        lr_T_mult = getattr(cfg.training, "lr_T_mult", 1)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=lr_T0, T_mult=lr_T_mult, eta_min=1e-6,
        )
    else:
        ...
        for step, (images, task_ids, targets) in enumerate(train_loader):
            ...
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=use_amp):
                ...
                loss, ldict = compute_loss(
                    ...
                    use_vfl=getattr(cfg.model, "use_vfl", True),
                )
