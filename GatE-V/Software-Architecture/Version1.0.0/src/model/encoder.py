...
class HybridEncoderBlock(nn.Module):
    ...
    def forward(self, src, src_mask=None, pos_embed=None, use_checkpoint=False):
        output = src
        for layer in self.layers:
            if use_checkpoint and self.training:
                output = torch.utils.checkpoint.checkpoint(
                    layer, output, src_mask, pos_embed, use_reentrant=False,
                )
            else:
...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.out_channels = [hidden_dim for _ in range(num_levels)]
        ...
        # Intra-scale encoder transformer (applied to P5 / last level)
        ...
        for _ in range(num_levels - 1, 0, -1):
        ...
        # AFF: Adaptive Feature Fusion — learnable per-level weights after PAN
        if use_aff:
            self.aff_weights = nn.Parameter(torch.ones(num_levels))
    ...
    def forward(self, feats: list[torch.Tensor], use_checkpoint: bool = False) -> list[torch.Tensor]:
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                cache_key = (h, w, src_flatten.device)
                if not hasattr(self, "_pos_cache") or self._pos_cache.get("key") != cache_key:
                    pos_embed = self.build_2d_sincos_position_embedding(
                        w, h, self.hidden_dim, self.pe_temperature
                    ).to(src_flatten.device)
                    if not hasattr(self, "_pos_cache"):
                        self._pos_cache = {"key": cache_key, "pe": pos_embed}
                    else:
                        self._pos_cache["key"] = cache_key
                        self._pos_cache["pe"] = pos_embed
                else:
                    pos_embed = self._pos_cache["pe"]
                memory = self.encoder[i](src_flatten, pos_embed=pos_embed, use_checkpoint=use_checkpoint)
                proj_feats[enc_ind] = (
        ...
        # AFF: apply learnable per-level weights after PAN
        if self.use_aff:
            outs = [outs[i] * self.aff_weights[i] for i in range(len(outs))]
