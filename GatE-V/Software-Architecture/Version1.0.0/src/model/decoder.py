...
class MSDeformableAttention(nn.Module):
    def __init__(self, embed_dim=256, num_heads=8, num_levels=3,
        ...
        # Use bilinear (default) during training for differentiability;
        # use configured method (discrete) during inference for FPGA
        self.attn_core_train = functools.partial(
            deformable_attention_core_func_v2, method="default")
        self.attn_core_eval = functools.partial(
            deformable_attention_core_func_v2, method=self.method)
        # Cache static tensors as buffers
        self.register_buffer("_repeats", torch.tensor(num_points_list, dtype=torch.long))
    ...
    def forward(self, query, reference_points, value,
        ...
        attention_weights = F.softmax(attention_weights, dim=-1).reshape(
            bs, Len_q, self.num_heads, sum(self.num_points_list))
        repeats = self._repeats.to(device=query.device)
        ref_points_expanded = torch.repeat_interleave(
            reference_points, repeats, dim=2)
        if reference_points.shape[-1] == 2:
            cache_key = (tuple(value_spatial_shapes.flatten().tolist()), query.device)
            offset_normalizer = self._offset_norm_cache.get(cache_key)
            if offset_normalizer is None:
                offset_normalizer = torch.tensor(
                    value_spatial_shapes, device=query.device).flip([1]).reshape(
                    1, 1, 1, self.num_levels, 2)
                self._offset_norm_cache[cache_key] = offset_normalizer
        ...
        attn_core = self.attn_core_train if self.training else self.attn_core_eval
        output = attn_core(
...
class RTDETRDecoderLayer(nn.Module):
    ...
    def forward(self, target, reference_points, memory,
        ...
        if task_bias is not None:
            target_mod = target_mod + task_bias.unsqueeze(1)
        q = k = self.with_pos_embed(target_mod, query_pos_embed)
        target2, _ = self.self_attn(q, k, value=target_mod, attn_mask=attn_mask)
        target = target_mod + self.dropout1(target2)
...
class RTDETRDecoder(nn.Module):
    """Iterative-refinement decoder with stable additive task bias."""
    def __init__(self, hidden_dim, decoder_layer, num_layers,
                 num_levels=3, eval_idx=-1, use_checkpoint=False):
        super().__init__()
        self.layers = nn.ModuleList(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.use_checkpoint and self.training:
                output = torch.utils.checkpoint.checkpoint(
                    ...
                    query_pos_embed, task_bias, use_reentrant=False,
                )
            else:
                output = layer(output, ref_points_input, memory,
                               memory_spatial_shapes, attn_mask, memory_mask,
                              query_pos_embed, task_bias=task_bias)
...
class RTDETRTransformerv2(nn.Module):
    ...
    def _get_decoder_input(self, memory, spatial_shapes, task_query_bias=None):
        ...
        return content, enc_topk_bbox_unact, enc_outputs_logits, enc_outputs_coord_unact

    def forward(self, feats, task_query_bias=None, task_bias_per_layer=None):
        memory, spatial_shapes = self._get_encoder_input(feats)
        content, init_ref, enc_outputs_logits, enc_outputs_coord_unact = self._get_decoder_input(
        ...
        out = {
            ...
            "enc_outputs": {
                ...
                "pred_boxes": F.sigmoid(enc_outputs_coord_unact)
            }
        }
