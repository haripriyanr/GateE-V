...
DVCon_2026/
├── DVCon_SoC_SRC/
│   ├── ACCELERATOR_IP/          ← GatE-V Systolic Array RTL
│   │   ├── gatev_top.sv         ← Top-level accelerator module
│   │   ├── gatev_mac_engine.sv  ← 32x16 PE array & SiLU activation LUT
│   │   ├── gatev_axi4_master.sv ← Double-buffered 64-bit DMA engine
│   │   ├── gatev_axi_lite_slave.sv ← Memory-mapped control registers (0x4000_0000)
│   │   ├── gatev_async_fifo.sv  ← Clock domain crossing FIFOs
│   │   └── gatev_pkg.sv         ← Global parameters & configuration
│   ├── TOP/                     ← VEGA AS1061 SoC Top Integration
│   ├── TB/                      ← Testbench environment & simulation models
│   └── MEMORY_IP/               ← BRAM & Boot ROM models (rom_32KB_axi)
├── Application/                 ← Bare-metal C application firmware
│   ├── gatev_firmware/          ← GatE-V reproduceable self-test (build.sh, main.c, Makefile)
│   ├── toolchain-bare/          ← RISC-V cross toolchain
│   └── Util/                    ← bin2hex / hex2mif / hex2mem conversion utilities
├── RUN/                         ← Automated simulation entry scripts (run.sh) + rom_32KB_axi.mif
├── TCL/                         ← Vivado synthesis scripts (DVCon_SYN.tcl)
├── GEN_BIT_OUT/                 ← Boot ROM (.coe) & waveform configuration (.wcfg)
...
- **Email:** team166.dvcon2026@bytesilicon.org
