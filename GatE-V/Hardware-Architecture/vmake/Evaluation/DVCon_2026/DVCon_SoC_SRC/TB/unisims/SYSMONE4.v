...
`endif
 ...
 //      SYSMON_VUSER3_MONITOR_NONE;
  ...
  // Total UNISIM %s- warning message next: 69
  ...
  assign busy_end_ed_wait = (adc_state == ST_A_WAIT_ED && adc_state_d != ST_A_WAIT_ED);
  ...
  assign cal_end_level     = (cal_counter==cal_limit-1); 
  ...
  always @(posedge adcclk or posedge sysmon_rst) begin
    if(sysmon_rst)
      ...
      if((conversion_before_calib && busy_end && adc_state!=ST_A_CALIB ) || adc_state==ST_A_WAIT_ED || adc_state==ST_A_WAIT)
        cal_counter <= 0;
      else if((adc_state==ST_A_FIRST_CALIB || adc_state==ST_A_CALIB) && cal_counter <= cal_limit-1)
  ...
  always @( posedge DCLK_in or posedge alm_rst or posedge gsr_in ) begin
    if(alm_rst ==1 || gsr_in==1) begin
      ...
      if (chan_out_id_cur == 0) begin // temperature 
        if (data_written[15:4] >= ot_limit_reg[15:4])
          ot_out_reg <= 1;
