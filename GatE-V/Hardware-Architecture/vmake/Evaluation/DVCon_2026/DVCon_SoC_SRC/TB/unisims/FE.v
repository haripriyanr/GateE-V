...
`ifdef XIL_DR
  `include "FE_dr.v"
`else
  localparam [6:0] LD_PERCENT_LOAD_REG = LD_PERCENT_LOAD;
  localparam [6:0] LE_PERCENT_LOAD_REG = LE_PERCENT_LOAD;
  localparam [152:1] SIM_DEVICE_REG = SIM_DEVICE;
  localparam [6:0] TD_PERCENT_LOAD_REG = TD_PERCENT_LOAD;
`endif
...
always @ (trig_attr) begin
  #1;
  if ((attr_test == 1'b1) ||
      ((LD_PERCENT_LOAD_REG < 0) || (LD_PERCENT_LOAD_REG > 100))) begin
    $display("Error: [Unisim %s-101] LD_PERCENT_LOAD attribute is set to %d.  Legal values for this attribute are 0 to 100. Instance: %m", MODULE_NAME, LD_PERCENT_LOAD_REG);
    attr_err = 1'b1;
  end
  
  if ((attr_test == 1'b1) ||
      ((LE_PERCENT_LOAD_REG < 0) || (LE_PERCENT_LOAD_REG > 100))) begin
    $display("Error: [Unisim %s-102] LE_PERCENT_LOAD attribute is set to %d.  Legal values for this attribute are 0 to 100. Instance: %m", MODULE_NAME, LE_PERCENT_LOAD_REG);
    attr_err = 1'b1;
  end
  ...
  if ((attr_test == 1'b1) ||
      ((TD_PERCENT_LOAD_REG < 0) || (TD_PERCENT_LOAD_REG > 100))) begin
    $display("Error: [Unisim %s-104] TD_PERCENT_LOAD attribute is set to %d.  Legal values for this attribute are 0 to 100. Instance: %m", MODULE_NAME, TD_PERCENT_LOAD_REG);
    attr_err = 1'b1;
  end
